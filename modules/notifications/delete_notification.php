...
if (is_numeric($notificationId)) {
    ...
    echo json_encode(array("status" => "success", "message" => "Benachrichtigung wurde entfernt"));
} else {
    if ($notificationId == $constants["all"]) {
        ...
        $deleteStatement->bindValue(':ownId', $own_id);
        $deleteStatement->execute();
        echo json_encode(array("status" => "success", "message" => "Alle Benachrichtigungen wurden entfernt"));
        ...
        $deleteStatement->bindValue(':ownId', $own_id);
        $deleteStatement->execute();
        echo json_encode(array("status" => "success", "message" => "Alle ungelesenen Benachrichtigungen wurden entfernt"));
    } else {
