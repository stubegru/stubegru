...
export default class SpamFilterService {
    ...
    async getAllSpamFilter() {
        const resp = await Stubegru.fetch.getJson("ts_modules/spam_filter/backend/get_all_spam_filter.php") as SpamFilter[];
        return resp;
    }

    async getSpamFilter(spamFilterId: string) {
        const resp = await Stubegru.fetch.getJson("ts_modules/spam_filter/backend/get_spam_filter.php", { id: spamFilterId }) as SpamFilter;
        return resp;
    }
    ...
    async deleteSpamFilter(spamFilterId: string) {
        let resp = await Stubegru.fetch.postJson("ts_modules/spam_filter/backend/delete_spam_filter.php", { id: spamFilterId }) as StubegruHttpResponse;
        if (resp.status == "error") { throw new Error(resp.message) };
        return resp;
    }
}
