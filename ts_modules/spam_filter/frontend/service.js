...
export default class SpamFilterService {
    async getAllSpamFilter() {
        const resp = await Stubegru.fetch.getJson("ts_modules/spam_filter/backend/get_all_spam_filter.php");
        return resp;
    }
    async getSpamFilter(spamFilterId) {
        const resp = await Stubegru.fetch.getJson("ts_modules/spam_filter/backend/get_spam_filter.php", { id: spamFilterId });
        return resp;
    }
    ...
    async deleteSpamFilter(spamFilterId) {
        let resp = await Stubegru.fetch.postJson("ts_modules/spam_filter/backend/delete_spam_filter.php", { id: spamFilterId });
        if (resp.status == "error") {
            throw new Error(resp.message);
        }
        ;
